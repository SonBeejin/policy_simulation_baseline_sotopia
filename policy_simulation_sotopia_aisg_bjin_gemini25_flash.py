...
프로젝트 루트에서 실행:
    python sotopia/policy_simulation_sotopia_aisg_bjin.py
=========================================================================
cd "C:\Users\BEEJIN\Desktop\비진이\HCCL\정책연구"
$env:PYTHONUTF8="1"
$env:PARALLEL_RUNS="7"
.\.venv\Scripts\python.exe sotopia/policy_simulation_sotopia_aisg_bjin_gemini25_flash.py
=========================================================================
출력 파일:
...
RUN_OUTPUT_DIR = RESULT_DIR / "policy_simulation_sotopia_aisg_bjin_gemini25_flash_runs"
OUTPUT_PATH = RUN_OUTPUT_DIR / "sotopia_simulation_aisg_results_all_runs.json"
INTERACTION_LOG_PATH = (
    RUN_OUTPUT_DIR / "sotopia_simulation_aisg_interaction_log_all_runs.json"
)
...
def deepseek_call(
    ...
    for attempt in range(LUXIA_MAX_RETRIES + 1):
        try:
            response = requests.post(
                LUXIA_API_URL, headers=headers, json=payload, timeout=120
            )
            ...
            if (
                status_code not in LUXIA_RETRY_STATUS_CODES
                or attempt >= LUXIA_MAX_RETRIES
            ):
    ...
    usage = {"prompt_tokens": int(usage_raw.get("promptTokenCount") or usage_raw.get("prompt_tokens") or usage_raw.get("input_tokens") or 0), "completion_tokens": int(usage_raw.get("candidatesTokenCount") or usage_raw.get("completion_tokens") or usage_raw.get("output_tokens") or 0)}
...
def build_policy_prompt(policy_text: str) -> str:
    return (
        ...
        "the other agents.\n\n"
        f"{policy_text}\n\n"
        "Target indicator: cumulative_ai_project_completion_count_2022. This is the "
        "cumulative number of AI projects completed published during "
        "the 2021-2022 period.\n\n"
        "Guidance:\n"
        "- If the policy document contains an explicit value or target, use "
        "that as the anchor.\n"
        "- If it does not, make a reasonable estimate from your professional "
        "background and the discussion evidence.\n"
        "- During discussion, compare assumptions about research scale, "
        "publication capacity, support structure, and field-specific output.\n\n"
...
class PolicyDiscussionAgent(BaseAgent[Observation, AgentAction]):
    def __init__(self, persona: dict[str, Any], policy_prompt: str) -> None:
        ...
        self._goal = (
            ...
            "a prediction for cumulative_ai_project_completion_count_2022."
        )
    ...
    async def aact(self, obs: Observation) -> AgentAction:
        ...
        if self.turns_spoken == 0:
            ...
            user_prompt = (
                ...
                "and keep the conversation focused on the cumulative AI project completion "
                "publication count for 2021-2022.\n\n"
    ...
    async def final_prediction(self, interaction_log: list[dict[str, Any]]) -> str:
        ...
        user_prompt = (
            ...
            '{"prediction_values": {"cumulative_ai_project_completion_count_2022": <number>}, '
...
def build_env_profile(num_agents: int) -> EnvironmentProfile:
    return EnvironmentProfile(
        codename="aisg_ai_project_completion_multi_agent_discussion",
        scenario=(
            "Multiple AISG stakeholders discuss the input policy document and "
            "predict the cumulative AI project completion count through 2022. "
            "Each participant speaks from their professional background and "
            "reacts to others."
        ),
        agent_goals=[
            "Provide an evidence-based estimate of "
            "cumulative_ai_project_completion_count_2022 and respond to other stakeholders."
...
async def run_once(
    ...
    output_path = RUN_OUTPUT_DIR / (
        f"sotopia_simulation_aisg_results_run_{run_index:03d}.json"
    )
    interaction_log_path = RUN_OUTPUT_DIR / (
        f"sotopia_simulation_aisg_interaction_log_run_{run_index:03d}.json"
    )
    ...
    for result in results:
        ...
        pred_val = (
            ...
            .get("cumulative_ai_project_completion_count_2022")
        )
    ...
    return {
        ...
        "prediction_key": "cumulative_ai_project_completion_count_2022",
