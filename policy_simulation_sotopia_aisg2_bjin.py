...
프로젝트 루트에서 실행:
    python sotopia/policy_simulation_sotopia_aisg2_bjin.py
=========================================================================
cd "C:\Users\BEEJIN\Desktop\비진이\HCCL\정책연구"
$env:PYTHONUTF8="1"
$env:PARALLEL_RUNS="50"
.\.venv\Scripts\python.exe sotopia/policy_simulation_sotopia_aisg2_bjin.py
=========================================================================
출력 파일:
...
RUN_OUTPUT_DIR = RESULT_DIR / "policy_simulation_sotopia_aisg2_bjin_runs"
...
DEEPSEEK_MAX_RETRIES = int(os.getenv("DEEPSEEK_MAX_RETRIES", "6"))
DEEPSEEK_RETRY_BASE_SECONDS = float(os.getenv("DEEPSEEK_RETRY_BASE_SECONDS", "5"))
DEEPSEEK_RETRY_STATUS_CODES = {429, 500, 502, 503, 504}
...
def deepseek_call(
    ...
    if not DEEPSEEK_API_KEY:
        raise RuntimeError("Set DEEPSEEK_API_KEY in this file before running.")
    ...
    for attempt in range(DEEPSEEK_MAX_RETRIES + 1):
        ...
        try:
            ...
            if status_code not in DEEPSEEK_RETRY_STATUS_CODES or attempt >= DEEPSEEK_MAX_RETRIES:
        ...
        sleep_seconds = min(DEEPSEEK_RETRY_BASE_SECONDS * (2 ** attempt), 120)
        print(
            "[deepseek retry] "
            f"attempt {attempt + 1}/{DEEPSEEK_MAX_RETRIES} failed: "
...
async def main() -> None:
    ...
    async def run_with_limit(run_index: int) -> dict[str, Any]:
        async with semaphore:
            try:
                ...
                print(
                    f"[simulation {run_index}/{NUM_RUNS}] error: "
    ...
    interaction_log_refs = [
        {
            "run_index": run_result.get("run_index"),
            "run_seed": run_result.get("run_seed"),
            "status": run_result.get("status", "success"),
            "interaction_log_path": run_result.get("interaction_log_path"),
