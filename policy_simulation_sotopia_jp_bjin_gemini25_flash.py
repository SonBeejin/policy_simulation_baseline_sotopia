...
프로젝트 루트에서 실행:
    python sotopia/policy_simulation_sotopia_jp_bjin_gemini25_flash.py
=========================================================================
cd "C:\Users\BEEJIN\Desktop\비진이\HCCL\정책연구"
$env:PYTHONUTF8="1"
$env:PARALLEL_RUNS="7"
.\.venv\Scripts\python.exe sotopia/policy_simulation_sotopia_jp_bjin_gemini25_flash.py
=========================================================================
출력 파일:
...
PERSONAS_PATH = BASE_DIR / "simulation_personas_jp.json"
POLICY_PATH = BASE_DIR / "input" / "jp_aip_2023.txt"
RESULT_DIR = BASE_DIR / "sotopia_result"
RUN_OUTPUT_DIR = RESULT_DIR / "policy_simulation_sotopia_jp_bjin_gemini25_flash_runs"
OUTPUT_PATH = RUN_OUTPUT_DIR / "sotopia_simulation_jp_aip_results_all_runs.json"
INTERACTION_LOG_PATH = (
    RUN_OUTPUT_DIR / "sotopia_simulation_jp_aip_interaction_log_all_runs.json"
)
...
def build_policy_prompt(policy_text: str) -> str:
    return (
        "We are simulating a stakeholder discussion about the input Japan AIP "
        ...
        "the other agents.\n\n"
        f"{policy_text}\n\n"
        "Target indicator: main_conference_accepted_articles_neurips_iclr_icml. "
        "This is the Number of articles accepted in the main machine learning "
        "conferences NeurIPS, ICLR, and ICML.\n\n"
        "Guidance:\n"
        "- If the policy document gives an explicit accepted-article value, "
        "target, or comparable baseline, use it as the main anchor.\n"
        "- If no explicit value is provided, estimate a reasonable count from "
        "the program's AI/ML research scale, research teams, funding, "
        "collaborations, publication pipeline, and your persona's expertise.\n"
        "- Count only accepted articles in NeurIPS, ICLR, and ICML; do not "
        "include journal papers, workshops, domestic conferences, or other "
...
class PolicyDiscussionAgent(BaseAgent[Observation, AgentAction]):
    def __init__(self, persona: dict[str, Any], policy_prompt: str) -> None:
        ...
        self._goal = ("Discuss Japan AIP policy effects with the other stakeholders and provide a prediction for accepted articles in NeurIPS, ICLR, and ICML.")
    ...
    async def aact(self, obs: Observation) -> AgentAction:
        ...
        if self.turns_spoken == 0:
            ...
            user_prompt = (
                ...
                "and keep the conversation focused on accepted articles in "
    ...
    async def final_prediction(self, interaction_log: list[dict[str, Any]]) -> str:
        ...
        user_prompt = (
            ...
            '{"prediction_values": {"main_conference_accepted_articles_neurips_iclr_icml": <number>}, '
...
def build_env_profile(num_agents: int) -> EnvironmentProfile:
    return EnvironmentProfile(
        codename="jp_aip_main_conference_articles_multi_agent_discussion",
        scenario=(
            "Multiple Japan AIP stakeholders discuss the input policy document "
            "and predict the number of accepted articles in NeurIPS, ICLR, "
            "and ICML. Each participant speaks from their professional "
            "background and reacts to others."
        ),
        agent_goals=[
            "Provide an evidence-based estimate of main_conference_accepted_articles_neurips_iclr_icml and respond to other stakeholders."
...
async def run_once(
    ...
    output_path = RUN_OUTPUT_DIR / (
        f"sotopia_simulation_jp_aip_results_run_{run_index:03d}.json"
    )
    interaction_log_path = RUN_OUTPUT_DIR / (
        f"sotopia_simulation_jp_aip_interaction_log_run_{run_index:03d}.json"
    )
    ...
    for result in results:
        ...
        pred_val = (
            ...
            .get("main_conference_accepted_articles_neurips_iclr_icml")
        )
    ...
    return {
        ...
        "prediction_key": "main_conference_accepted_articles_neurips_iclr_icml",
