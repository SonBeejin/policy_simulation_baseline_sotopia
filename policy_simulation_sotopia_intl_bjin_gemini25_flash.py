...
프로젝트 루트에서 실행:
    python sotopia/policy_simulation_sotopia_intl_bjin.py

=========================================================================
cd "C:\Users\BEEJIN\Desktop\비진이\HCCL\정책연구"
$env:PYTHONUTF8="1"
$env:PARALLEL_RUNS="5"
.\.venv\Scripts\python.exe sotopia/policy_simulation_sotopia_intl_bjin.py
=========================================================================
출력 파일:
    sotopia_simulation_intl_results.json
    sotopia_simulation_intl_interaction_log.json
"""
...
RUN_OUTPUT_DIR = RESULT_DIR / "policy_simulation_sotopia_intl_bjin_gemini25_flash_runs"
OUTPUT_PATH = RUN_OUTPUT_DIR / "sotopia_simulation_intl_results_all_runs.json"
INTERACTION_LOG_PATH = (
    RUN_OUTPUT_DIR / "sotopia_simulation_intl_interaction_log_all_runs.json"
)
...
def build_policy_prompt(policy_text: str) -> str:
    return (
        ...
        f"{policy_text}\n\n"
        "Target indicator: international_co_research_ratio. This is the "
        "percentage of research outputs involving international co-research "
        "among BK21 research outputs.\n\n"
...
class PolicyDiscussionAgent(BaseAgent[Observation, AgentAction]):
    def __init__(self, persona: dict[str, Any], policy_prompt: str) -> None:
        ...
        self._goal = (
            ...
            "a prediction for international_co_research_ratio."
        )
    ...
    async def aact(self, obs: Observation) -> AgentAction:
        ...
        if self.turns_spoken == 0:
            ...
            user_prompt = (
                ...
                "and keep the conversation focused on the international "
                "co-research ratio.\n\n"
    ...
    async def final_prediction(self, interaction_log: list[dict[str, Any]]) -> str:
        ...
        user_prompt = (
            ...
            '{"prediction_values": {"international_co_research_ratio": <number>}, '
...
def build_env_profile(num_agents: int) -> EnvironmentProfile:
    return EnvironmentProfile(
        codename="bk21_intl_multi_agent_discussion",
        scenario=(
            "Multiple BK21 stakeholders discuss the 2022 BK21 policy plan and "
            "predict the international co-research ratio. Each participant "
            "speaks from their professional background and reacts to others."
        ),
        agent_goals=[
            "Provide an evidence-based estimate of international_co_research_ratio "
            "and respond to other stakeholders."
...
async def run_once(
    ...
    output_path = RUN_OUTPUT_DIR / (
        f"sotopia_simulation_intl_results_run_{run_index:03d}.json"
    )
    interaction_log_path = RUN_OUTPUT_DIR / (
        f"sotopia_simulation_intl_interaction_log_run_{run_index:03d}.json"
    )
    ...
    for result in results:
        ...
        pred_val = (
            ...
            .get("international_co_research_ratio")
        )
    ...
    return {
        ...
        "prediction_key": "international_co_research_ratio",
