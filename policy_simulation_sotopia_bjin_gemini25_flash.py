...
프로젝트 루트에서 실행:
    python sotopia/policy_simulation_sotopia_bjin_gemini25_flash.py

=========================================================================
cd "C:\Users\BEEJIN\Desktop\비진이\HCCL\정책연구"
$env:PYTHONUTF8="1"
$env:PARALLEL_RUNS="5"
.\.venv\Scripts\python.exe sotopia/policy_simulation_sotopia_bjin_gemini25_flash.py
=========================================================================
출력 파일:
...
PERSONAS_PATH = BASE_DIR / "simulation_personas_bk21.json"
POLICY_PATH = BASE_DIR / "bk21_2022_plan.txt"
RESULT_DIR = BASE_DIR / "sotopia_result"
RUN_OUTPUT_DIR = RESULT_DIR / "policy_simulation_sotopia_bjin_gemini25_flash_runs"
OUTPUT_PATH = RUN_OUTPUT_DIR / "sotopia_simulation_results_all_runs.json"
INTERACTION_LOG_PATH = (
    RUN_OUTPUT_DIR / "sotopia_simulation_interaction_log_all_runs.json"
)
...
def build_policy_prompt(policy_text: str) -> str:
    return (
        "We are simulating a stakeholder discussion about the 2022 BK21 policy "
        "plan. Based on the policy summary below and your professional persona, "
        "discuss the policy outcome indicator with the other agents.\n\n"
        f"{policy_text}\n\n"
        "Target indicator: beneficiary_student_employment_rate. This is the "
        "employment rate (%) of graduate beneficiary students supported by "
        "BK21 after graduation.\n\n"
        "Guidance:\n"
        "- If the policy document contains an explicit target or value, use "
        "that as the anchor.\n"
        "- If it does not, make a reasonable estimate from your professional "
        "background and the discussion evidence.\n"
        "- During discussion, compare assumptions about graduate employment, "
        "industry linkage, research training, and field-specific placement.\n\n"
...
class PolicyDiscussionAgent(BaseAgent[Observation, AgentAction]):
    def __init__(self, persona: dict[str, Any], policy_prompt: str) -> None:
        ...
        self._goal = (
            "Discuss BK21 policy effects with the other stakeholders and provide "
            "a prediction for beneficiary_student_employment_rate."
        )
    ...
    async def aact(self, obs: Observation) -> AgentAction:
        ...
        if self.turns_spoken == 0:
            ...
            user_prompt = (
                ...
                "and keep the conversation focused on the beneficiary student "
                "employment rate.\n\n"
    ...
    async def final_prediction(self, interaction_log: list[dict[str, Any]]) -> str:
        ...
        user_prompt = (
            ...
            '{"prediction_values": {"beneficiary_student_employment_rate": <number>}, '
...
def build_env_profile(num_agents: int) -> EnvironmentProfile:
    return EnvironmentProfile(
        codename="bk21_employment_multi_agent_discussion",
        scenario=(
            "Multiple BK21 stakeholders discuss the 2022 BK21 policy plan and "
            "predict the beneficiary student employment rate. Each participant "
            "speaks from their professional background and reacts to others."
        ),
        agent_goals=[
            "Provide an evidence-based estimate of "
            "beneficiary_student_employment_rate and respond to other stakeholders."
...
async def run_once(
    ...
    output_path = RUN_OUTPUT_DIR / (
        f"sotopia_simulation_results_run_{run_index:03d}.json"
    )
    interaction_log_path = RUN_OUTPUT_DIR / (
        f"sotopia_simulation_interaction_log_run_{run_index:03d}.json"
    )
    ...
    for result in results:
        ...
        pred_val = (
            ...
            .get("beneficiary_student_employment_rate")
        )
    ...
    return {
        ...
        "prediction_key": "beneficiary_student_employment_rate",
