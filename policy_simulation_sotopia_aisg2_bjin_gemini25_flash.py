...
프로젝트 루트에서 실행:
    python sotopia/policy_simulation_sotopia_aisg2_bjin_gemini25_flash.py
=========================================================================
cd "C:\Users\BEEJIN\Desktop\비진이\HCCL\정책연구"
$env:PYTHONUTF8="1"
$env:PARALLEL_RUNS="7"
.\.venv\Scripts\python.exe sotopia/policy_simulation_sotopia_aisg2_bjin_gemini25_flash.py
=========================================================================
출력 파일:
...
PERSONAS_PATH = BASE_DIR / "simulation_personas_100e.json"
POLICY_PATH = BASE_DIR / "input" / "AISG.txt"
RESULT_DIR = BASE_DIR / "sotopia_result"
RUN_OUTPUT_DIR = RESULT_DIR / "policy_simulation_sotopia_aisg2_bjin_gemini25_flash_runs"
OUTPUT_PATH = RUN_OUTPUT_DIR / "sotopia_simulation_aisg2_results_all_runs.json"
INTERACTION_LOG_PATH = (
    RUN_OUTPUT_DIR / "sotopia_simulation_aisg2_interaction_log_all_runs.json"
)

LUXIA_API_URL = os.getenv(
    "LUXIA_GEMINI25_FLASH_API_URL",
    "https://bridge.luxiacloud.com/llm/google/gemini/generate/flash20/content",
)
LUXIA_API_KEY = os.getenv("LUXIA_API_KEY", "")
MODEL_NAME = os.getenv("LUXIA_GEMINI25_FLASH_MODEL", "gemini-2.5-flash")
...
def _extract_gemini_text(data: dict) -> str:
    ...
    if isinstance(response, dict):
        nested = _extract_gemini_text(response)
        if nested:
            return nested
    chunks: list[str] = []
...
def deepseek_call(
    ...
    payload = {"model": MODEL_NAME, "contents": contents}
    response: requests.Response | None = None
    last_error: Exception | None = None
    for attempt in range(LUXIA_MAX_RETRIES + 1):
        try:
            response = requests.post(LUXIA_API_URL, headers=headers, json=payload, timeout=120)
    ...
    if response is None:
        raise RuntimeError("Luxia request failed without a response.")
    data = response.json()
    text = _extract_gemini_text(data)
    usage_raw = data.get("usageMetadata") or data.get("usage") or {}
    usage = {
        "prompt_tokens": int(usage_raw.get("promptTokenCount") or usage_raw.get("prompt_tokens") or usage_raw.get("input_tokens") or 0),
        "completion_tokens": int(usage_raw.get("candidatesTokenCount") or usage_raw.get("completion_tokens") or usage_raw.get("output_tokens") or 0),
    }
    usage["total_tokens"] = int(usage_raw.get("totalTokenCount") or usage_raw.get("total_tokens") or (usage["prompt_tokens"] + usage["completion_tokens"]))
    with _token_lock:
        _total_calls += 1
...
def build_policy_prompt(policy_text: str) -> str:
    return (
        "We are simulating a stakeholder discussion about the input AISG "
        "policy document. Based on the policy document below and your "
        "professional persona, discuss the policy outcome indicator with "
        "the other agents.\n\n"
        f"{policy_text}\n\n"
        "Target indicator: deployed_ai_project_completion_count_2022. This "
        "counts only AI projects that were both completed and actually "
        "deployed or adopted in an industrial field, company, production "
        "process, service, or real operational setting by the end of 2022, "
        "counted since 2018.\n\n"
        "Important distinction:\n"
        "- Do not count projects that were merely selected, funded, started, "
        "performed, ongoing, planned, prototyped, demonstrated, or completed "
        "on paper unless the result was deployed/adopted for real industry use.\n"
        "- If the policy document gives separate counts for performed projects "
        "and industry-applied/deployed projects, use only the industry-applied "
        "or deployed count.\n"
        "- Treat performed/supported/selected/R&D/completed project counts as "
        "broader categories, not the target, unless industry application is "
        "explicitly stated.\n"
        "- If no explicit deployed/applied value is provided, estimate a "
        "reasonable industry-applied count from deployment/adoption evidence, "
        "policy goals, project pipeline, funding scale, implementation timeline, "
        "and your persona's expertise.\n\n"
...
class PolicyDiscussionAgent(BaseAgent[Observation, AgentAction]):
    def __init__(self, persona: dict[str, Any], policy_prompt: str) -> None:
        ...
        self._goal = (
            "Discuss AISG policy effects with the other stakeholders and provide "
            "a prediction for completed AI projects actually deployed or adopted "
            "in industry through 2022."
        )
    ...
    async def aact(self, obs: Observation) -> AgentAction:
        ...
        if self.turns_spoken == 0:
            ...
            user_prompt = (
                ...
                "and keep the conversation focused on completed AI projects actually "
                "deployed or adopted in industry through 2022.\n\n"
    ...
    async def final_prediction(self, interaction_log: list[dict[str, Any]]) -> str:
        ...
        user_prompt = (
            ...
            '{"prediction_values": {"deployed_ai_project_completion_count_2022": <number>}, '
...
def build_env_profile(num_agents: int) -> EnvironmentProfile:
    return EnvironmentProfile(
        codename="aisg2_deployed_ai_project_multi_agent_discussion",
        scenario=(
            "Multiple AISG stakeholders discuss the input policy document and "
            "predict the count of completed AI projects that were actually "
            "deployed or adopted in industry through 2022. Each participant "
            "speaks from their professional background and reacts to others."
        ),
        agent_goals=[
            "Provide an evidence-based estimate of "
            "deployed_ai_project_completion_count_2022 and respond to other stakeholders."
...
async def run_once(
    ...
    output_path = RUN_OUTPUT_DIR / (
        f"sotopia_simulation_aisg2_results_run_{run_index:03d}.json"
    )
    interaction_log_path = RUN_OUTPUT_DIR / (
        f"sotopia_simulation_aisg2_interaction_log_run_{run_index:03d}.json"
    )
    ...
    for result in results:
        ...
        pred_val = (
            ...
            .get("deployed_ai_project_completion_count_2022")
        )
    ...
    return {
        ...
        "prediction_key": "deployed_ai_project_completion_count_2022",
